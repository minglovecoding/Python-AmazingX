...
void solve() {
    ...
    planted.reserve(N); //planted的容量为N
    stk.reserve(N);
...
- 先把最终状态（第 Q 天）的所有带子建好，然后把“网格外一圈”标为能出网。
- 从四周边界出发调用 `dfs`，把能出网性向内传播，求出 `ans[q]=tot`（第 Q 天能出网总数）。
- 再倒序从第 Q 天删回到第 2 天：每删掉一天的带子，就把该格子变成 `?`，它立刻可能变成“能出网”，于是以它为源再跑一次 `dfs`（会递归把受影响的点标好），记录 `ans[i-1]`。
- 最后打印每一天的最少不可用格子数 `n*n - ans[i]`。
...
// tot：当前“能出网”的格子数量
// ans[i]：第 i 天后的最小不可用数量（稍后打印 n*n - ans[i]）
// g[x][y]：格子类型；-1=未建('?')；0=U,1=R,2=D,3=L
// s[x][y]：是否“能出网”（1=能；0=未知/不能）
struct Node{
    int x,y,d;
}a[Q];
...
void dfs(int x,int y){
    if(s[x][y]) return; // 已经标记为“能出网”，直接返回

    bool flag=false;
    if(g[x][y]==-1){ // 未建 '?'
        // 只要四邻有一个已是“能出网”，它也可以朝那个邻居建，从而“能出网”
        for(int i=0;i<4;i++){
        ...
        // 只看“下一格”是否“能出网”（走出网格也算，见下初始化）
        if(s[nx][ny]) flag=true;
    }
    if(!flag) return;  // 目前还不能判定它能出网，先不标记

    ++tot;             // 现在 (x,y) 也能出网
    s[x][y]=1;

    // 它变成“能出网”后，可能让它的邻居也变得“能出网”，继续尝试四邻
    for(int i=0;i<4;i++){
        int nx=x+dx[i],ny=y+dy[i];
...
int main(){
  ...
  // 在“第Q天”的最终状态下，从四条边尝试启动 dfs，向内传播“能出网”
    for(int i=1;i<=n;i++){
        dfs(1,i);
        dfs(n,i);
        dfs(i,1);
...
int main() {
    ...
    set<string> moos;  // save different moos

    auto find_moos = [&](const string &s) {
        unordered_map<string, int> cnt; //hash map
        for (int i = 0; i + 2 < N; ++i) {
            if (s[i] != s[i + 1] && s[i + 1] == s[i + 2]) {
                string t = s.substr(i, 3); //substr
