...
##### 4.1 0-1 背包
 ...
 给定容量为 `W` 的背包，n 件物品，每件有容量 `w[i]` 和价值 `v[i]`。
 要求在不超过容量的情况下，价值最大。
...
dp[5]=dp[4]+2=2 dp[4]=dp[3]+2=2 dp[3]=dp[2]+2=2 dp[2]=dp[1]+2=2 dp[1]=2
dp[5]=dp[3]+4=6 ...dp[2]=dp[0]+4=4
dp[5]=...
...
### 🌲 示例4：树的重心（Tree Centroid）

### 📘 1. 定义

> **树的重心**：删除该节点后，使剩余每棵子树大小最大值最小的点。
...
- `sz[u]` = 以 u 为根的子树大小
- `max_sub[u]` = 删除 u 之后，剩余最大的那一块的节点数

计算：

```
max_sub[u] = max( 所有子树的 sz[v], n - sz[u] )
```

最终答案是：**max_sub[u] 最小的节点 u**
...
- 对 1：最大子树 = 2 → max_sub[1] = 2
- 对 4：最大子树 = max(sz[5]=1, n-sz[4]=3) → 3
- 对 2：max_sub[2] = n-sz[2] = 4
- 对 3：max_sub[3] = 4
- 对 5：max_sub[5] = 4

最小的是节点 1，因此 **节点 1 是重心**。
...
### 🌲 示例5：树的路径和（Maximum Path Sum / Tree Path DP）

这类 DP 与“树的直径”非常类似，但一般加上“节点权值”，变成最大权路径。
...
- `down[u] = 从 u 向下走的最大路径和`

- 全局：

  ```c++
  ans = max(ans, down[v1] + down[v2] + val[u]) // 两条子树合并
  ```

转移：

```c++
down[u] = max(0, max(down[v] + val[u] for all v in children))
```
...
```
path_through_3 = 1(来自4→3) + 4 = 5
```

在节点 1：

```
path_through_1 = 0(来自2) + 5(来自3) + 3 = 8
```
